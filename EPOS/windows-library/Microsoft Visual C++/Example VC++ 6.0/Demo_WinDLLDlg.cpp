...
BOOL CDemo_WinDLLDlg::UpdateStatus()
{
    ...
    if(oResult)
    {
        oResult = VCS_GetOperationMode(m_KeyHandle, m_usNodeId, &m_uMode, &m_ulErrorCode);
        if(oResult)
        {
            switch(m_uMode)
            {
...
BOOL CDemo_WinDLLDlg::RestoreEPOS()
{
    ...
    if(m_oInitialisation)
    {
        ...
        VCS_SetOperationMode(m_KeyHandle, m_usNodeId, m_uMode, &m_ulErrorCode);
...
BOOL CDemo_WinDLLDlg::OpenDevice()
{
    ...
    if(hNewKeyHandle)
    {
        ...
        {
            //Read Operation Mode
            if(VCS_GetOperationMode(m_KeyHandle, m_usNodeId, &m_uMode, &m_ulErrorCode))
            {
                ...
                if(VCS_GetPositionProfile(m_KeyHandle, m_usNodeId, &m_ulProfileVelocity, &m_ulProfileAcceleration, &m_ulProfileDeceleration, &m_ulErrorCode))
                {
                    ...
                    if(VCS_SetOperationMode(m_KeyHandle, m_usNodeId, PROFILE_POSITION_MODE, &m_ulErrorCode))
                    {
...
void CDemo_WinDLLDlg::OnChangeEditNodeId() 
{
    ...
    if(m_strNodeId != "")
    {
        m_usNodeId = atoi(m_strNodeId);
        if(m_usNodeId >= 1 && m_usNodeId <= 128)
        {
            if(SetTimer(1, 100, NULL)) m_oUpdateActive = TRUE;
        }
        else
...
************************************************************************/
BOOL CDemo_WinDLLDlg::ShowErrorInformation(DWORD ulErrorCode)
{
    char* pStrErrorInfo;
    ...
    if(VCS_GetErrorInfo(ulErrorCode, pStrErrorInfo, 100))
    {
